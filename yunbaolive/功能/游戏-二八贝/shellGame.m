...
-(void)startGame:(playSuccess)ok{
    //service=Game.Jinhua
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"service=%@",_zhajinhuakaishi];
    NSDictionary *subdic = @{
    ...
    [session POST:url parameters:subdic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqual:@"0"])
            {
                ...
                _info = info;
                [self.delagate prepGame:[info valueForKey:@"gameid"] ndMethod:_method andMsgtype:_msgtype andBanklist:nil];
...
-(void)resulttimeraction{
    ...
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"service=%@",_zhajinhuaresult];
    NSDictionary *subdic = @{
                             @"uid":[Config getOwnID],
                             @"gameid":_gameid
                             };
...
-(void)getShellCoin:(NSString *)type andMoney:(NSString *)money{
    ...
    if (all >= 10000) {
        all = all/10000;
        btns2.text = [NSString stringWithFormat:@"%.1fW",all];
    }else{
...
-(void)getNewCOins:(NSArray *)array{
    for (int i=0; i<3; i++) {
        ...
        if (single >= 10000) {
            single = single/10000;
            btns2.text = [NSString stringWithFormat:@"%.1fW",single];
        }else{
