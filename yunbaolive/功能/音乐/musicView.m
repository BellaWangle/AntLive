...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([manager fileExistsAtPath:loadPath]) {
        ...
        NSString *url = [oldPurl stringByAppendingFormat:@"?service=Music.getDownurl&audio_id=%@",[dic valueForKey:@"audio_id"]];
        ...
        [session GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            ...
            if ([ret isEqual:@"200"]) {
                ...
                if ([code isEqual:@"0"]) {
                    ...
                    //下载歌曲
                    NSString *path = [NSString stringWithFormat:@"%@",[info valueForKey:@"audio_link"]];
                    cell.path = path;
                    cell.songName = [NSString stringWithFormat:@"%@",[dic valueForKey:@"lrc_title"]];
                    [self.tableView reloadData];
                    //保存歌词
                    NSString *lrcLink = [NSString stringWithFormat:@"%@",[info valueForKey:@"lrc_content"]];
...
-(void)searchBarSearchButtonClicked:(UISearchBar *)searchBar{
    ...
    music = 1;
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [oldPurl stringByAppendingFormat:@"?service=Music.searchMusic&key=%@",self.search.text];
     url = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    [session GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        NSString *ret = [NSString stringWithFormat:@"%@",[responseObject valueForKey:@"ret"]];
        if ([ret isEqual:@"200"]) {
