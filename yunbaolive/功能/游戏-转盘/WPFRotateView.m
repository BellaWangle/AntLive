...
-(void)cancle{
    ...
    NSString *url = [purl stringByAppendingFormat:@"service=Game.Dial_end&stream=%@&liveuid=%@&token=%@&type=%@&gameid=%@&ifset=%@",[NSString stringWithFormat:@"%@",_stream],[Config getOwnID],[Config getOwnToken],@"2",_gameid,@"1"];
    NSLog(@"%@",url);
...
-(void)getRotationCoinType:(NSString *)type andMoney:(NSString *)money{
    ...
    if (all >= 1000) {
        all = all/1000;
        btns.badgeValue = [NSString stringWithFormat:@"%.1fK",all];
    }
    else{
...
-(void)stakePokers:(UIButton *)sender{
    ...
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"service=Game.Dial_Bet"];
    NSDictionary *subdic = @{
                             @"stream":_stream,
                             @"uid":[Config getOwnID],
    ...
    [session POST:url parameters:subdic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqual:@"0"])
            {
                ...
                if (all >= 1000) {
                    all = all/1000;
                    labels.text = [NSString stringWithFormat:@"%.1fK",all];
                }
                else{
...
-(void)getResultINternet{
    if (!_ishost) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
            NSString *url = [purl stringByAppendingFormat:@"service=Game.settleGame"];
            NSDictionary *subdic = @{
                                     @"uid":[Config getOwnID],
                                     @"gameid":_gameid
                                     };
...
- (IBAction)doRotation:(id)sender {
    ...
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"service=Game.Dial"];
    NSDictionary *subdic = @{
                             @"stream":_stream,
    ...
    [session POST:url parameters:subdic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            {
                NSDictionary *info = [[data valueForKey:@"info"] firstObject];
                [self.delegate startRotationGameSocketToken:[info valueForKey:@"token"] andGameID:[info valueForKey:@"gameid"] andTime:[info valueForKey:@"time"]];
...
-(void)jijiangkaishi{
    ...
    if (!imageVS) {
        ...
        [imageVS setTitle:YZMsg(@"即将开始") forState:UIControlStateNormal];
...
-(void)continueGame:(NSString *)time andgameId:(NSString *)gameid andMoney:(NSArray *)moneys andmycoin:(NSArray *)moneyss{
    ...
    for (int i=0; i<4; i++) {
        ...
        if (all >0) {
            ...
            if (all >= 1000) {
                all = all/1000;
                btn.badgeValue = [NSString stringWithFormat:@"%.1fK",all];
            }
            else{
...
-(void)getnewrecord{
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"service=%@&stream=%@&action=%@",@"Game.getGameRecord",_stream,@"3"];
    [session POST:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSNumber *number = [responseObject valueForKey:@"ret"] ;
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
