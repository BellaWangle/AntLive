...
@interface AntPersonMsg ()<UITableViewDataSource,UITableViewDelegate,UIAlertViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDelegate,UICollectionViewDataSource>
{
...
@end
@implementation AntPersonMsg
-(NSArray *)models{
    
    NSMutableArray *array = [NSMutableArray array];
    for (NSDictionary *dic in self.aArray) {
        backLiveModel *model = [backLiveModel modelWithDic:dic];
...
{
    backLiveCell *cell = [backLiveCell cellWithTV:tableView];
    backLiveModel *model = self.models[indexPath.row];
...
- (IBAction)pushLiveVC:(id)sender {
    ...
    for (UIViewController *tempVc in self.navigationController.viewControllers) {
        if ([tempVc isKindOfClass:[anchorRoomVC class]]) {
            [HUDHelper myalert:YZMsg(@"请先退出直播在观看")];
            return;
        }
    }
    for (UIViewController *tempVc in self.navigationController.viewControllers) {
        if ([tempVc isKindOfClass:[SpectatorRoomVC class]]) {
            isLivePlay = YES;
            temp = tempVc;
        }
    }
...
-(void)pushMovieVC{
    SpectatorRoomVC *pl = [[SpectatorRoomVC alloc]init];
    pl.playDoc = movieDic;
    pl.isJpush = @"1";
