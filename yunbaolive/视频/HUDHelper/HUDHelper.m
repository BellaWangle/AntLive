...
{
    [HUDHelper alert:msg cancel:YZMsg(@"确认")];
}
+ (void)myalert:(NSString *)msg
{
    [HUDHelper alert:msg cancel:nil];
}
+ (void)alert:(NSString *)msg action:(CommonVoidBlock)action
{
    [HUDHelper alert:msg cancel:YZMsg(@"确认") action:action];
}
+ (void)alert:(NSString *)msg cancel:(NSString *)cancel
{
    [HUDHelper alertTitle:YZMsg(@"提示") message:msg cancel:cancel];
}
+ (void)alert:(NSString *)msg cancel:(NSString *)cancel action:(CommonVoidBlock)action
{
    [HUDHelper alertTitle:YZMsg(@"提示") message:msg cancel:cancel action:action];
}
...
{
    UIView *inView = view ? view : [MXBADelegate sharedAppDelegate].window;
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        UIView *inView = [MXBADelegate sharedAppDelegate].window;
        MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:inView];
        if (![NSString isEmpty:msg])
        {
...
- (void)tipMessage:(NSString *)msg delay:(CGFloat)seconds completion:(void (^)())completion
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        MBProgressHUD *hud = [[MBProgressHUD alloc] initWithWindow:[MXBADelegate sharedAppDelegate].window];
        [[MXBADelegate sharedAppDelegate].window addSubview:hud];
        hud.mode = MBProgressHUDModeText;
        hud.labelText = msg;
        [hud show:YES];
        [hud hide:YES afterDelay:seconds];
