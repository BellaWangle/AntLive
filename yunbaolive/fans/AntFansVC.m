...
@interface  AntFansVC()<UITableViewDelegate,UITableViewDataSource,guanzhu>
{
    NSInteger a;
    int setvisfans;
    AntPersonMsg *person;
...
-(void)request
{
    ...
    NSString *url = [purl stringByAppendingFormat:@"service=User.getFansList&uid=%@&touid=%@&p=%@",[Config getOwnID],self.fensiUid,@"1"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    person = [[AntPersonMsg alloc]init];
    AntFansModel *model = self.fansmodels[indexPath.row];
