...
-(void)setModel:(AntFansModel *)model{
    _model = model;
...
+(AntFansCell *)cellWithTableView:(UITableView *)tv{
    AntFansCell *cell = [tv dequeueReusableCellWithIdentifier:@"a"];
    if (!cell) {
       cell = [[NSBundle mainBundle]loadNibNamed:@"AntFansCell" owner:self options:nil].lastObject;
    }
    return cell;
