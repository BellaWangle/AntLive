...
@end
@implementation personRankList
-(void)setview{
    UIView *navtion = [[UIView alloc]initWithFrame:CGRectMake(0, 0, _window_width,64+aaaaa)];
    ...
    UIButton *bigBTN = [[UIButton alloc]initWithFrame:CGRectMake(0, aaaaa, _window_width/3,60)];
    [bigBTN addTarget:self action:@selector(doReturn) forControlEvents:UIControlEventTouchUpInside];
    [navtion addSubview:bigBTN];
    returnBtn.frame = CGRectMake(10,30+aaaaa,30,20);[returnBtn.imageView setContentMode:UIViewContentModeScaleAspectFit];
    ...
    segmentC.frame = CGRectMake(_window_width/4,25+aaaaa,_window_width/2,30);
...
-(void)change:(UISegmentedControl *)segment{
    if (segment.selectedSegmentIndex == 0) {
        ...
        NSString *userBaseUrl = [h5url stringByAppendingFormat:@"g=appapi&m=Contribute&a=order&type=week&uid=%@",_userID];
        NSURL *url = [NSURL URLWithString:userBaseUrl];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        [wevView loadRequest:request];
        wevView.backgroundColor = [UIColor whiteColor];
        wevView.frame = CGRectMake(0,64+aaaaa, _window_width, _window_height-64 - aaaaa);
        ...
        wevView = [[UIWebView alloc] init];
        NSString *userBaseUrl = [h5url stringByAppendingFormat:@"g=appapi&m=Contribute&a=order&type=all&uid=%@",_userID];
        NSURL *url = [NSURL URLWithString:userBaseUrl];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        [wevView loadRequest:request];
        wevView.backgroundColor = [UIColor whiteColor];
