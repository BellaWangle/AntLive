...
#import "Config.h"
@interface equipmentVC ()<UIWebViewDelegate>
{
    UIWebView *wevView;
    NSURL *url;
    //四个下划线
...
-(void)navtion{
    ...
    UISegmentedControl *segment = [[UISegmentedControl alloc]initWithItems:@[YZMsg(@"坐骑"),YZMsg(@"靓号")]];
...
-(void)change:(UISegmentedControl *)segment{
    ...
    if (segment.selectedSegmentIndex == 0) {
        [wevView stopLoading];
        //坐骑
        NSString *paths = [h5url stringByAppendingFormat:@"g=Appapi&m=Car&a=mycar&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
    ...
    //靓号
        ...
        [wevView stopLoading];
        NSString *paths = [h5url stringByAppendingFormat:@"g=Appapi&m=Liang&a=myliang&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
        url = [NSURL URLWithString:paths];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        [wevView removeFromSuperview];
        wevView = nil;
