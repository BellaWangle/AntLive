...
#import "Config.h"
@interface antLiveMarket ()<UIWebViewDelegate>
{
    UIWebView *wevView;
    NSURL *url;
    //四个下划线
...
-(void)navtion{
    ...
    UISegmentedControl *segment = [[UISegmentedControl alloc]initWithItems:@[YZMsg(@"会员"),YZMsg(@"靓号"),YZMsg(@"坐骑")]];
...
-(void)change:(UISegmentedControl *)segment{
    if (segment.selectedSegmentIndex == 0) {
        [wevView stopLoading];
        //商城
        NSString *paths = [h5url stringByAppendingFormat:@"g=Appapi&m=Vip&a=index&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
    ...
    //坐骑
        ...
        [wevView stopLoading];
        NSString *paths = [h5url stringByAppendingFormat:@"g=Appapi&m=Car&a=index&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
        url = [NSURL URLWithString:paths];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        [wevView removeFromSuperview];
        wevView = nil;
    ...
    //靓号
        ...
        [wevView stopLoading];
        NSString *paths = [h5url stringByAppendingFormat:@"g=Appapi&m=Liang&a=index&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
        url = [NSURL URLWithString:paths];
        NSURLRequest *request = [NSURLRequest requestWithURL:url];
        [wevView removeFromSuperview];
        wevView = nil;
