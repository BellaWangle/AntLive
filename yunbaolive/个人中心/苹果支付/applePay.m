...
-(void)applePay:(NSDictionary *)dic{
    ...
    NSString *oidurl = [oldPurl stringByAppendingFormat:@"/?service=Charge.getIosOrder"];
    NSDictionary *dics = @{
                          @"uid":[Config getOwnID],
    ...
    [session POST:oidurl parameters:dics progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if ([ret isEqual:@200]) {
            ...
            if ([code isEqual:@0]) {
                NSString *infos = [[[data valueForKey:@"info"] firstObject] valueForKey:@"orderid"];
...
- (void) verifyReceipt:(SKPaymentTransaction *)transaction
{
    ...
    [self.delegate applePayHUD];
    NSURL *url = [NSURL URLWithString:[payUrl stringByAppendingFormat:@"/Appapi/Pay/notify_ios"]];
