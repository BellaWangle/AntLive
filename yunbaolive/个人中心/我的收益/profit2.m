...
- (void)viewDidLoad {
   ...
   // input.layer.cornerRadius = 20;
    ...
    [tixian setTitle:YZMsg(@"提现") forState:UIControlStateNormal];
...
-(void)tixian{
    ...
    if (input.text.length<=0 || [input.text isEqualToString:@"0"]) {
        ...
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"请输入金额" message:nil delegate:self
                                             cancelButtonTitle:YZMsg(@"确认") otherButtonTitles:nil, nil];
    ...
    [session GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqualToNumber:[NSNumber numberWithInt:0]])
            {
                ...
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:info message:nil delegate:self cancelButtonTitle:YZMsg(@"确认") otherButtonTitles:nil, nil];
                [alert show];
                input.text = @" ";
                ...
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:msg message:nil delegate:self cancelButtonTitle:YZMsg(@"确认") otherButtonTitles:nil, nil];
...
-(void)navtion{
    ...
    returnBtn.tintColor = [UIColor clearColor];
