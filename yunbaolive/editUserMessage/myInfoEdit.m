...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        eidtIcon *icon = [[eidtIcon alloc]initWithNibName:@"eidtIcon" bundle:nil];
        [[MXBADelegate sharedAppDelegate] pushViewController:icon animated:YES];
    }
    else {
        if(indexPath.row == 0)
        {
        nickNameEdie *EditNameView = [[nickNameEdie alloc] init];
            [[MXBADelegate sharedAppDelegate] pushViewController:EditNameView animated:YES];
        
        }
        else if(indexPath.row == 1){
        WriteSignature *EditSignatureView = [[WriteSignature alloc] init];
