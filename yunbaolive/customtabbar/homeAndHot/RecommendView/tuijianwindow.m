...
-(void)pullInternetforNew{
    NSString *url;
    url = [purl stringByAppendingFormat:@"service=Home.getRecommend&uid=%@",[Config getOwnID]];
...
-(void)doatten{
    ...
    NSString *url;
    url = [purl stringByAppendingFormat:@"service=Home.attentRecommend&uid=%@&touid=%@",[Config getOwnID],_touids];
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    [session POST:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
