...
#import "AntLiveRankCell.h"

@interface AntLiveRankVC ()<UITableViewDelegate,UITableViewDataSource> {
...
-(void)pullData {
    ...
    [AntPullData pullWithUrl:postUrl Dic:postDic Suc:^(id data, id code, id msg) {
        if ([code isEqual:@"0"]) {
...
-(NSArray *)models {
    ...
    for (NSDictionary *dic in _dataArray) {
        AntLiveRankModel *model = [AntLiveRankModel modelWithDic:dic];
...
-(void)clickFollowBtn:(UIButton *)btn {
    ...
    btn.enabled = NO;
    AntLiveRankModel *model = [_models objectAtIndex:btn.tag - 202];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    AntLiveRankModel *model = _models[indexPath.row];
    AntLiveRankCell *cell = [AntLiveRankCell cellWithTab:tableView indexPath:indexPath];
    //收益榜-0 消费榜-1
    if (segment1.selectedSegmentIndex==0) {
        model.type = @"0";
    }else{
