...
@implementation AntLiveRankCell
...
+(AntLiveRankCell *)cellWithTab:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath{
    AntLiveRankCell *cell;
    if (indexPath.row == 0) {
        cell = [tableView dequeueReusableCellWithIdentifier:@"firstCell"];
        if (!cell) {
            cell = [[[NSBundle mainBundle]loadNibNamed:@"AntLiveRankCell" owner:nil options:nil]objectAtIndex:0];
        }
        ...
        if (!cell) {
            cell = [[[NSBundle mainBundle]loadNibNamed:@"AntLiveRankCell" owner:nil options:nil]objectAtIndex:1];
        }
        cell.iconIV.layer.masksToBounds = YES;
