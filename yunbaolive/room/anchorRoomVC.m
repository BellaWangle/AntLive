...
-(void)getZombieList:(NSArray *)list{
    ...
    if (!listView) {
        listView = [[roomUserListView alloc]initWithListArray:list andID:[Config getOwnID] andStream:[NSString stringWithFormat:@"%@",[self.roomDic valueForKey:@"stream"]]];
...
- (void)viewDidLoad {
    ...
    __block anchorRoomVC *weakself = self;
...
-(void)startUI{
    ...
    listView = [[roomUserListView alloc]initWithListArray:nil andID:[Config getOwnID] andStream:[NSString stringWithFormat:@"%@",[_roomDic valueForKey:@"stream"]]];
    listView.frame = CGRectMake(110,20+sssss,_window_width-130,40);
    listView.delegate = self;
    [frontView addSubview:listView];
...
-(void)OnChoseFilter:(id)sender {
    ...
    if (!beautifulgirl) {
        __weak anchorRoomVC *weakself = self;
...
//    pushBTN.backgroundColor = normalColors;
    ...
    useraimation = [[SpecialEffect alloc]init];
    useraimation.frame = CGRectMake(10,self.tableView.top - 40,_window_width,20);
...
-(void)doupcoast{
    if (!coastview) {
        coastview = [[coastView alloc]initWithFrame:CGRectMake(0, -_window_height, _window_width, _window_height) andsureblock:^(NSString *type) {
            coastmoney = type;
...
//跳往魅力值界面
-(void)yingpiao{
    personRankList *jumpC = [[personRankList alloc] init];
...
-(void)showmoreviews{
    ...
    __weak anchorRoomVC *weakself = self;
    if (!bottombtnV) {
        bottombtnV = [[roomFunctionV alloc]initWithFrame:CGRectMake(0,_window_height*2, _window_width, _window_width/2) music:^(NSString *type) {
...
-(void)pushZhuYe:(NSString *)IDS{
    [self doCancle];
    AntPersonMsg  *person = [[AntPersonMsg alloc]initWithNibName:@"AntPersonMsg" bundle:nil];
    person.userID = IDS;
    person.isLIve = 1;
...
-(void)pushCoinV{
    AntCoinVeiw *coin = [[AntCoinVeiw alloc] init];
...
{
    jingpaiweb = [[BiddersWebview alloc]initWithFrame:CGRectMake(0, 0, _window_width, _window_height) andblock:^(NSString *type) {
        //发送竞拍消息
...
-(void)getjingpaimessagedelegate:(NSDictionary *)dic{
    ...
    if (!hostjingpai) {
        ...
        hostjingpai = [[BiddersVC alloc]initWithFrame:CGRectMake(_window_width - _window_width*0.4, 90, _window_width*0.4, 50) andblock:^(NSDictionary *dic) {
            isjingpaiwriting = NO;
