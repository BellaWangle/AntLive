...
-(void)createroom{
    ...
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"service=Live.createRoom&uid=%@&token=%@&user_nicename=%@&title=%@&province=%@&city=%@&lng=%@&lat=%@&type=%d&type_val=%@",[Config getOwnID],[Config getOwnToken],[Config getOwnNicename],title,@"",[locationCache getMyCity],[locationCache getMylng],[locationCache getMylat],liveType,type_vals];
    ...
    [session POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if ([code isEqual:@"0"]) {
            ...
            anchorRoomVC *lives = [[anchorRoomVC alloc]init];
            lives.type = liveType;
