...
@interface coastView ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
...
-(instancetype)initWithFrame:(CGRect)frame andsureblock:(axinblock)sureblock andcancleblock:(axinblock)cancleblock{
    ...
    if (self) {
        ...
        for (int i=0; i<arrays.count; i++) {
            NSString *path = [NSString stringWithFormat:@"%@%@/%@",arrays[i],[common name_coin],YZMsg(@"分")];
        ...
        NSArray *titles = @[@"选择收费",YZMsg(@"取消"),YZMsg(@"确认")];
        
        for (int i = 0; i<3; i++) {
            UIButton *btn = [UIButton buttonWithType:0];
            btn.titleLabel.font = fontThin(13);
            if (i == 0) {
                [btn setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@.jpg",getImagename(@"docoasessssimage")]] forState:UIControlStateNormal];
...
-(void)doactions:(UIButton *)sender{
    if ([sender.titleLabel.text isEqual:YZMsg(@"确认")]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    timeChargeCell *cell = [timeChargeCell cellWithTableView:tableView];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    timeChargeCell *cell =  (timeChargeCell *)[self.tableView cellForRowAtIndexPath:indexPath];
    cell.showlabel.textColor = normalColors;
