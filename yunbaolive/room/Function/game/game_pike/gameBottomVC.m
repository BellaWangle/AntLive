...
-(void)cancle{
    ...
    NSString *url = [purl stringByAppendingFormat:@"service=%@&stream=%@&liveuid=%@&token=%@&type=%@&gameid=%@&ifset=%@",_zhajinhuaendgame,[NSString stringWithFormat:@"%@",[self.zhuboDic valueForKey:@"stream"]],[Config getOwnID],[Config getOwnToken],@"2",[_info valueForKey:@"gameid"],@"1"];
...
-(instancetype)initWIthDic:(NSDictionary *)dic andIsHost:(BOOL)ok andMethod:(NSString *)method andMsgtype:(NSString *)msgtype{
    ...
    if (self) {
        ...
        if ([_method isEqual:@"startGame"]) {
            ...
            _personnameX =@[[NSString stringWithFormat:@"%@X2",YZMsg(@"大乔")],[NSString stringWithFormat:@"%@X2",YZMsg(@"貂蝉")],[NSString stringWithFormat:@"%@X2",YZMsg(@"小乔")]];
        ...
        if([_method isEqual:@"startCattleGame"]) {
            ...
            _personnameX =@[[NSString stringWithFormat:@"%@X2",YZMsg(@"星星")],[NSString stringWithFormat:@"%@X2",YZMsg(@"球球")],[NSString stringWithFormat:@"%@X2",YZMsg(@"仔仔")]];
        ...
        if ([_method isEqual:@"startLodumaniGame"]) {
            ...
            _personnameX = @[[NSString stringWithFormat:@"%@X2",YZMsg(@"罗伯茨")],[NSString stringWithFormat:@"%@X2",YZMsg(@"平局")],[NSString stringWithFormat:@"%@X2",YZMsg(@"基德")]];
...
-(void)startGame:(playSuccess)ok{
    ...
    //service=Game.Jinhua
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"service=%@&stream=%@&liveuid=%@&token=%@",_zhajinhuakaishi,[self.zhuboDic valueForKey:@"stream"],[Config getOwnID],[Config getOwnToken]];
    ...
    [session POST:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqual:@"0"])
            {
                ...
                NSDictionary *bankerlist = [info valueForKey:@"bankerlist"];
                _info = info;
                [self.delagate prepGame:[info valueForKey:@"gameid"] ndMethod:_method andMsgtype:_msgtype andBanklist:bankerlist];
                  if([_method isEqual:@"startCattleGame"]) {
                ...
                _gameid = [info valueForKey:@"gameid"];
                cutDownNums = [[info valueForKey:@"time"] intValue];
                [self createUI];
                ok(code);
            }
            else{
...
-(void)kaijiang{
    NSString *url;
    if (!_isHost) {
        url = [purl stringByAppendingFormat:@"service=%@&uid=%@&gameid=%@",_zhajinhuaresult,[Config getOwnID],_gameid];
    }
    else{
        url = [purl stringByAppendingFormat:@"service=%@&uid=%@&gameid=%@",_zhajinhuaresult,[Config getOwnID],[_info valueForKey:@"gameid"]];
    }
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    [session POST:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSNumber *number = [responseObject valueForKey:@"ret"] ;
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            {
                NSDictionary *info = [[data valueForKey:@"info"] firstObject];
                NSString *coin = [NSString stringWithFormat:@"%@",[info valueForKey:@"coin"]];
                NSString *banker_profit = [NSString stringWithFormat:@"%@",[info valueForKey:@"banker_profit"]];
                NSString *gamecoin = [NSString stringWithFormat:@"%@",[info valueForKey:@"gamecoin"]];
                NSString *isshow = [NSString stringWithFormat:@"%@",[info valueForKey:@"isshow"]];
                
                NSArray *keyArray = [info allKeys];
                if ([keyArray containsObject:@"isshow"]) {
                    
                if (![isshow isEqual:@"0"]) {
                    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"" message:[NSString stringWithFormat:@"%@%@%@",YZMsg(@"您的余额不足"),[gameState getzhuanglimit],YZMsg(@"将被下庄")] delegate:self cancelButtonTitle:YZMsg(@"确认") otherButtonTitles:nil, nil];
                    [alert show];
                }
             }
                //刷新我自己的余额
                ...
                if (shangzhuang) {
                    ...
                    NSString *gamecoin = [NSString stringWithFormat:@"%@",[info valueForKey:@"gamecoin"]];
                    LiveUser *user = [Config myProfile];
                    user.coin = [NSString stringWithFormat:@"%@",[info valueForKey:@"coin"]];
                    [Config updateProfile:user];
                    int succcess = [gamecoin intValue];
                    if (succcess >0) {
                        NSString *money = [NSString stringWithFormat:@"%@%@",gamecoin,[common name_coin]];
                        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:YZMsg(@"赢了哦") message:money delegate:self cancelButtonTitle:YZMsg(@"确认") otherButtonTitles:nil, nil];
                        [alert show];
                    }
                    else {
                        //没中奖
...
-(void)stakePokers:(UIButton *)sender{
    ...
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"service=%@",_zhajinhuagameyazhu];
    ...
    [session POST:url parameters:subdic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqual:@"0"])
            {
                ...
                NSDictionary *info = [[data valueForKey:@"info"] firstObject];
                [self chongzhiV:[NSString stringWithFormat:@"%@",[info valueForKey:@"coin"]]];
                LiveUser *user = [Config myProfile];
                user.coin = [NSString stringWithFormat:@"%@",[info valueForKey:@"coin"]];
                [Config updateProfile:user];
...
-(void)getCoinType:(NSString *)type andMoney:(NSString *)money{
    ...
    if (all >= 10000) {
        all = all/10000;
        btns.allCoinLabel.text = [NSString stringWithFormat:@"%.1fW",all];
    }else{
...
-(void)getnewrecord{
    ...
    NSString *url = [purl stringByAppendingFormat:@"service=%@&stream=%@&action=%@",@"Game.getGameRecord",[self.zhuboDic valueForKey:@"stream"],_gameaction];
