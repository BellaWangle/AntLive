...
-(void)doitok{
    ...
    if ([game_bankerid isEqual:@"0"] || [game_bankerid isEqual:@"<null>"]) {
        [_iconbtn setImage:[UIImage imageNamed:@"吕布头像.png"] forState:UIControlStateNormal];
    }
    else{
        [_iconbtn sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[_zhuangdic valueForKey:@"avatar"]]] forState:UIControlStateNormal];
    }
...
-(void)getbanksCoin:(NSDictionary *)zhuangdic{
    ...
    if ([game_bankerid isEqual:@"0"] || [game_bankerid isEqual:@"<null>"]) {
        ...
        [_iconbtn sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[zhuangdic valueForKey:@"avatar"]]] forState:UIControlStateNormal];
    }
            ...
            [session POST:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                ...
                if([number isEqualToNumber:[NSNumber numberWithInt:200]])
                {
                    ...
                    if([code isEqual:@"0"])
                    {
                        ...
                        [MBProgressHUD showSuccess:[NSString stringWithFormat:@"%@",[info valueForKey:@"msg"]]];
                        [self getnewrecord];
                    }
                    else{
                        ...
                        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                            ...
                            panduanalert = [[UIAlertView alloc]initWithTitle:YZMsg(@"上庄押金") message:@"" delegate:self cancelButtonTitle:YZMsg(@"取消") otherButtonTitles:YZMsg(@"确认"), nil];
...
-(void)dorecord{
    ...
    */
    ...
    if ([recordbtn.titleLabel.text isEqual:YZMsg(@"申请上庄")]) {
        ...
        url = [purl stringByAppendingFormat:@"service=Game.quietBanker&stream=%@&uid=%@",_stream,[Config getOwnID]];
        url = [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    [session POST:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSNumber *number = [responseObject valueForKey:@"ret"] ;
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqual:@"0"])
            {
                ...
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:[NSString stringWithFormat:@"%@",[data valueForKey:@"msg"]] message:@"" delegate:self cancelButtonTitle:YZMsg(@"确认") otherButtonTitles:nil, nil];
                [alert show];
            }
        }
        else{
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:[NSString stringWithFormat:@"%@",[responseObject valueForKey:@"msg"]] message:@"" delegate:self cancelButtonTitle:YZMsg(@"确认") otherButtonTitles:nil, nil];
            [alert show];
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
...
-(void)getnewrecord{
    ...
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"service=%@&stream=%@",@"Game.getBanker",_stream];
    [session POST:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSNumber *number = [responseObject valueForKey:@"ret"] ;
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqual:@"0"])
            {
                NSMutableArray *info = [NSMutableArray arrayWithArray:[data valueForKey:@"info"]];
                _items = info;
                if (_items.count >= 2) {
...
-(void)getliushui{
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *url = [purl stringByAppendingFormat:@"service=%@&stream=%@&uid=%@",@"Game.getBankerProfit",_stream,[_zhuangdic valueForKey:@"id"]];
    [session POST:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSNumber *number = [responseObject valueForKey:@"ret"] ;
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqual:@"0"])
            {
                ...
                _itemsliushui = info;
                NSDictionary *subdic = @{
                                         @"banker_card":YZMsg(@"牌型"),
                                          @"banker_profit":YZMsg(@"结算")
                                        };
