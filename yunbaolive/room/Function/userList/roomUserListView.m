...
#import "listModel.h"
#import "Config.h"
@interface roomUserListView ()<UICollectionViewDelegate,UICollectionViewDataSource>
{
...
-(void)listReloadNoew{
    ...
    NSString *url = [purl stringByAppendingFormat:@"service=Live.getUserLists"];
    NSDictionary *userlist = @{
                               @"liveuid":IDs,
                               @"stream":stream
                               };
  [session POST:url parameters:userlist progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            {
                NSArray *info = [[data valueForKey:@"info"] firstObject];
                NSArray *list = [info valueForKey:@"userlist"];
...
-(instancetype)initWithListArray:(NSMutableArray *)list andID:(NSString *)ID andStream:(NSString *)streams{
    ...
    if (self) {
        ...
        [_listCollectionview registerClass:[roomUserCell class] forCellWithReuseIdentifier:@"list"];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    roomUserCell *cell = (roomUserCell *)[collectionView dequeueReusableCellWithReuseIdentifier:@"list" forIndexPath:indexPath];
