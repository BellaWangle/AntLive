...
-(void)getover{
    ...
    NSString *url = [purl stringByAppendingFormat:@"service=Live.auctionEnd&uid=%@&token=%@&auctionid=%@",[Config getOwnID],[Config getOwnToken],jingpaiID];
    ...
    [session GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            {
                NSDictionary * singleUserArray = [[data valueForKey:@"info"] firstObject];
...
-(void)addjingpairesultview:(int)a anddic:(NSDictionary *)dic{
    ...
    if (a == 4){
        ...
        NSString *path = [NSString stringWithFormat:@"%@%@",YZMsg(@"最终出价"),minstr([dic valueForKey:@"bid_price"])];
