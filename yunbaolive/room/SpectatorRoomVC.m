...
@end
int d =1;
@implementation SpectatorRoomVC
{
...
-(void)addZombieByArray:(NSArray *)array{
    if (!listView) {
        listView = [[roomUserListView alloc]initWithListArray:_listArray andID:[self.playDoc valueForKey:@"uid"]andStream:[NSString stringWithFormat:@"%@",[self.playDoc valueForKey:@"stream"]]];
...
-(void)doFenxiang:(UIButton *)sender{
    if (!fenxiangV) {
        //分享弹窗
        fenxiangV = [[shareThirdView alloc]initWithFrame:CGRectMake(0,_window_height - _window_width/6 - 20, _window_width, _window_width/6 + 20)];
...
-(void)setView{
    ...
    setFrontV = [[playerView alloc] initWithDic:self.playDoc];
...
-(void)lastView{
    ...
    lastv = [[liveEndView alloc]initWithFrame:self.view.bounds block:^(NSString *nulls) {
...
-(void)getNodeJSInfo
{
    ...
    [socketDelegate setnodejszhuboDic:self.playDoc Handler:^(id arrays) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
           ...
           //userlist_time 间隔时间
               //获取用户列表
           listView = [[roomUserListView alloc]initWithListArray:[info valueForKey:@"userlists"] andID:[self.playDoc valueForKey:@"uid"] andStream:[NSString stringWithFormat:@"%@",[self.playDoc valueForKey:@"stream"]]];
           listView.delegate = self;
...
-(void)startConnectvideo{
    ...
    __weak SpectatorRoomVC *weakself = self;
