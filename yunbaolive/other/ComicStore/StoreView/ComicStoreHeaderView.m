...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:CGRectMake(0, 0, _window_width, _window_height)]) {
...
- (void)resetScrollViewContentLocation{
    ...
    self.scrollView.contentOffset = CGPointMake(_window_width, 0);
}
...
- (void)scrollViewAddContentView{
    ...
    for (int i = 0; i < 3; i++) {
        CGRect rect = CGRectMake(i * _window_width, 0, _window_width, _window_height);
...
- (UIScrollView *)scrollView{
    if (_scrollView == nil) {
        ...
        self.scrollView.contentSize = CGSizeMake(3 * _window_width, 0);
