...
- (void)badgeInit
{
    ...
    _smallLabels  = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 10, 10)];
    _smallLabels.textColor = [UIColor clearColor];
    [self addSubview:_smallLabels];
    _mysmallLabels  = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 10, 10)];
    _mysmallLabels.textColor = [UIColor clearColor];
    [self addSubview:_mysmallLabels];
}
- (void)refreshBadge
{
    // 更新属性
...
- (void)updateBadgeFrame
{
    ...
    self.badge.frame = CGRectMake(self.badgeOriginX/1.5, self.badgeOriginY, minWidth + padding, minHeight + padding);
    self.badge.layer.cornerRadius = 10;
    self.badge.layer.masksToBounds = YES;
}
// 角标值变化
