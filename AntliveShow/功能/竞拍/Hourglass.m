...
-(instancetype)initWithDic:(NSDictionary *)hostdic andFrame:(CGRect)frame Block:(taskblocks)block andtask:(taskblocks)taskblock andjingpaixiangqingblock:(taskblocks)jingpaiblock andchongzhi:(taskblocks)chongzhiblock{
    ...
    if (self) {
        ...
        [shangpinxiangqing setTitle:YZMsg(@"商品详情") forState:UIControlStateNormal];
...
-(void)dojingpai{
    ...
    NSString *userBaseUrl = [purl stringByAppendingFormat:@"service=Live.setAuction&uid=%@&token=%@&auctionid=%@",[Config getOwnID],[Config getOwnToken],idtype];
    
 [session POST:userBaseUrl parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqual:@"0"])
            {
                ...
                self.blocks(minstr([info valueForKey:@"bid_price"]));
                [MBProgressHUD showSuccess:YZMsg(@"竞拍投注成功")];
            }
            else {
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"" message:minstr([data valueForKey:@"msg"]) delegate:self cancelButtonTitle:YZMsg(@"取消") otherButtonTitles:YZMsg(@"确认"), nil];
                [alert show];
            }
        }
...
-(void)getcoins{
    ...
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *userBaseUrl = [purl stringByAppendingFormat:@"service=Live.getCoin&uid=%@&token=%@",[Config getOwnID],[Config getOwnToken]];
    [session POST:userBaseUrl parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSNumber *number = [responseObject valueForKey:@"ret"];
        NSArray *data = [responseObject valueForKey:@"data"];
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
...
-(void)addtimeL{
    ...
    timenamel.adjustsFontSizeToFitWidth = YES;
...
-(void)addjingpairesultview:(int)a anddic:(NSDictionary *)dic{
    ...
    if (a == 3) {
        ...
        if ([ID isEqual:[Config getOwnID]]) {
            ...
            NSString *path = [NSString stringWithFormat:@"%@%@",YZMsg(@"最终出价"),minstr([dic valueForKey:@"money"])];
...
-(void)paydaojishi{
    //付款倒计时
    paytimes-=1;
    timels.text = [NSString stringWithFormat:@"%@ %@ %@",YZMsg(@"剩余"),[self timeFormatted:paytimes],YZMsg(@"自动关闭")];
    if (paytimes<=0) {
        [MBProgressHUD showError:YZMsg(@"付款时间已过，付款失败")];
        [paytimer invalidate];
        paytimer = nil;
        [self dopaycancle];
        return;
...
-(void)addPayview:(NSDictionary *)dic{
    ...
    timels.adjustsFontSizeToFitWidth = YES;
    line = [[UILabel alloc]init];
    paymentbtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [paymentbtn setTitle:YZMsg(@"付款") forState:UIControlStateNormal];
    paymentbtn.titleLabel.adjustsFontSizeToFitWidth = YES;
...
-(void)dopay{
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    NSString *userBaseUrl = [purl stringByAppendingFormat:@"service=Live.setBidPrice&uid=%@&token=%@&auctionid=%@",[Config getOwnID],[Config getOwnToken],idtype];
    [session POST:userBaseUrl parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSNumber *number = [responseObject valueForKey:@"ret"];
        NSArray *data = [responseObject valueForKey:@"data"];
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
