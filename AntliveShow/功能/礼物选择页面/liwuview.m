    ...
    [session POST:url parameters:giftDic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:200]])
        {
            ...
            if([code isEqualToNumber:[NSNumber numberWithInt:0]])
            {
                ...
                [self chongzhiV:coin];
                if (isfree == 1) {
                    liwucell *cell = (liwucell *)[_collectionView cellForItemAtIndexPath:freeIndex];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    if (isfree == 1) {
        if (indexPath.item >= freeArr.count) {
            ...
            liwuModel *model = freeArr[indexPath.item];
            cell.model = model;
             ...
             if([number isEqualToNumber:[NSNumber numberWithInt:200]])
             {
                 ...
                 if([code isEqualToNumber:[NSNumber numberWithInt:0]])
                 {
                     ...
                     for (NSDictionary *dic in info) {
                         liwuModel *model = [liwuModel modelWithDic:dic];
